...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      this.setState({ loaded: true, tokenSaleAddress: this.myTokenSaleInstance._address}, this.updateUserTokens);
    } catch (error) {
  ...
  listenToTokenTransfer = () => {
    this.myTokenInstance.events.Transfer({to: this.accounts[0]}).on("data",this.updateUserTokens);
  }
  ...
  render() {
    ...
    return (
      <div className="App">
          ...
          address to allow: <input text="text" name="kycAddress" value={this.state.kycAddress} onChange={this.handleInputChange} />
